...
namespace {
static constexpr std::size_t MIN_SECTOR_COUNT = 2;
static constexpr std::size_t MIN_STACK_COUNT = 2;
} // namespace

Sphere::Sphere(float radius, std::size_t sectors, std::size_t stacks) noexcept
...
void Sphere::build_vertices() {
  ...
  for (std::size_t i = 0; i <= stacks; i++) {
    ...
    for (std::size_t k = 0; k <= sectors; k++) {
      ...
      vertices.push_back(glm::vec3(glm::cos(phi) * glm::cos(theta) * radius,
